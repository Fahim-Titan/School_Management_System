...
{{--{{isset($value->sub_id1)}} taken by {{isset($value->t_id1)}}--}}
    {{--{{$course->sub_id1}}--}}
    @if($course->sub_id1 != 100 || $course->sub_id1 != NULL)
        {{$course->sub_id1}} is taken by {{$course->t_id1}}<br>
    @endif
    @if($course->sub_id2 != 100 || $course->sub_id2 != NULL)
        {{$course->sub_id2}} is taken by {{$course->t_id2}}<br>
    @endif
        @if($course->sub_id3 != 100 || $course->sub_id3 !=NULL)
        {{$course->sub_id3}} is taken by {{$course->t_id3}}<br>
        @endif
        @if($course->sub_id4 != 100 || $course->sub_id4 !=NULL)
